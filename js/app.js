...
function savePlayer(id, addToTourId) {
  ...
  if (id) {
    ...
    // Propaga nome e divisão para todos os torneios via gid = playerId
    const gid = G.players[i]?.playerId || id;
    G.tours.forEach(t => {
      t.players.forEach(tp => {
        if (tp.gid === gid || tp.gid === id) {
          tp.name     = data.name;
...
function saveArchModal(oldName) {
  ...
  // Normalize any existing objects back to strings
  G.settings.archetypes = G.settings.archetypes.map(a => typeof a === 'object' ? a.name : a);
  if (oldName) {
    G.tours.forEach(t => {
